...
def fetch_job_recommendations_with_similarity(user_data:UserInputData):
    ...
    try:
        ...
        query = "SELECT job_data FROM original"
        cursor.execute(query)
        results = cursor.fetchall()
        ...
        for row in results:
            ...
            # 기술 태그 리스트 추출
            technical_tags = [tag["name"] for tag in job_position.get("technicalTags", [])]

            # 공고 데이터 리스트 생성
            job_list.append({
                "title": job_position.get("title"),
                "company": job_position.get("company", {}).get("name"),
                "url": "https://career.programmers.co.kr" + job_position.get("url"),
                "skills": " ".join(technical_tags),  # 기술 태그를 문자열로 변환
                "deadline": job_position.get("endAt")
            })
        ...
        # 사용자의 기술 스택을 문자열로 변환
        user_skills_str = " ".join(user_data)
